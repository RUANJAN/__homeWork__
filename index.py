...
def mybmi(self, name, birthday, height, weight):
    bmi =  weight/((height/100)**2)

    if bmi < 18.5:
        print(f"{name}您的ＢＭＩ為{bmi:.2f}, 體重太輕了！多吃點")

    elif 18.5 <= bmi < 24:
        print(f"{name}您的ＢＭＩ為{bmi:.2f}, 體重正常！")

    elif 24 <= bmi < 27:
        print(f"{name} 您的ＢＭＩ為{bmi:.2f}, 體重有點胖！")

    elif 27 <= bmi < 30:
        print(f"{name}您的ＢＭＩ為{bmi:.2f},找專業人士來減肥！")

    elif 30<= bmi < 35:
        print (f"{name}您的ＢＭＩ為{bmi:2f},體重太胖了～！！快點找醫生減肥！")

    else:    
        print (f"{name}您的ＢＭＩ維{bmi: .2f},胖死了快減肥呀！")
